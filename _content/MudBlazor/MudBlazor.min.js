...
if(typeof container.destroy==='function'){container.destroy();}}};class MudSplitPanel{static build(containerId,horizontal,resetOnDoubleClick,minPanelSize,firstPanelInitialSize,panelGap){window.splitPanels[containerId]=new MudSplitPanel(containerId,horizontal,resetOnDoubleClick,minPanelSize,firstPanelInitialSize,panelGap);}
constructor(containerId,horizontal,resetOnDoubleClick,minPanelSize,firstPanelInitialSize,panelGap){this.container=document.getElementById(containerId);if(!this.container){console.warn(`MudSplitPanel:Container with id'${containerId}'not found.`);return;}
const children=this.container.children;if(children.length!==3){console.warn(`MudSplitPanel:Invalid child count'${children.length}'.`);return;}
this.firstPanel=children[0];this.divider=children[1];this.secondPanel=children[2];this.isDragging=false;this.startPos=0;this.startFirstSize=0;this.lastTap=0;this.firstPanelInitialSize=firstPanelInitialSize;this.keyboardStep=10;this._onMouseDown=this._onMouseDown.bind(this);this._onMouseMove=this._onMouseMove.bind(this);this._onMouseUp=this._onMouseUp.bind(this);this._onDoubleClick=this._onDoubleClick.bind(this);this._onTouchEnd=this._onTouchEnd.bind(this);this._onKeyDown=this._onKeyDown.bind(this);this.divider.addEventListener("mousedown",this._onMouseDown);this.divider.addEventListener("touchstart",this._onMouseDown);this.divider.addEventListener("dblclick",this._onDoubleClick);this.divider.addEventListener("touchend",this._onTouchEnd);this.divider.addEventListener("keydown",this._onKeyDown);this.update(horizontal,resetOnDoubleClick,minPanelSize,panelGap,true);}
destroy(){this.divider.removeEventListener("mousedown",this._onMouseDown);this.divider.removeEventListener("touchstart",this._onMouseDown);this.divider.removeEventListener("dblclick",this._onDoubleClick);this.divider.removeEventListener("touchend",this._onTouchEnd);this.divider.removeEventListener("keydown",this._onKeyDown);this._onMouseUp();delete window.splitPanels[this.container.id];}
update(horizontal,resetOnDoubleClick,minPanelSize,panelGap,forceRecalculateSize=false){let shouldRecalculateSize=horizontal!==this.horizontal||forceRecalculateSize;this.horizontal=horizontal;this.minPanelSize=minPanelSize;this.panelGap=panelGap;this.resetOnDoubleClick=resetOnDoubleClick;this.divider.style.minWidth=this.horizontal?null:`${panelGap}px`;this.divider.style.minHeight=this.horizontal?`${panelGap}px`:null;let containerSize=this._getContainerSize();this.divider.ariaValueMin=(this.minPanelSize/containerSize*100).toFixed(2).toString();this.divider.ariaValueMax=((containerSize-panelGap-this.minPanelSize)/containerSize*100).toFixed(2).toString();if(shouldRecalculateSize){this.resetSizes();}}
resetSizes(firstPanelSize=null){this.firstPanel.style.width="100%";this.secondPanel.style.width="100%";this.firstPanel.style.height="100%";this.secondPanel.style.height="100%";const firstPanelSizeNew=firstPanelSize!==null?firstPanelSize:this.firstPanelInitialSize;if(firstPanelSizeNew!==null){this._setPanelSizes(firstPanelSizeNew,this._getContainerSize());}else{this.divider.ariaValueNow="50";}}
getDividerPosition(){return this.horizontal?this.firstPanel.clientHeight:this.firstPanel.clientWidth;}
setDividerPosition(offset){this.resetSizes(offset);}
_getContainerSize(){return this.horizontal?this.container.offsetHeight:this.container.offsetWidth;}
_onMouseDown(e){e.preventDefault();this.isDragging=true;const clientX=e.touches?e.touches[0].clientX:e.clientX;const clientY=e.touches?e.touches[0].clientY:e.clientY;if(this.horizontal){this.startPos=clientY;this.startFirstSize=this.firstPanel.offsetHeight;document.body.style.cursor="n-resize";}else{this.startPos=clientX;this.startFirstSize=this.firstPanel.offsetWidth;document.body.style.cursor="ew-resize";}
document.body.style.userSelect="none";document.addEventListener("mousemove",this._onMouseMove);document.addEventListener("touchmove",this._onMouseMove);document.addEventListener("mouseup",this._onMouseUp);document.addEventListener("touchend",this._onMouseUp);}
_onMouseMove(e){if(!this.isDragging)return;const clientX=e.touches?e.touches[0].clientX:e.clientX;const clientY=e.touches?e.touches[0].clientY:e.clientY;const delta=this.horizontal?clientY-this.startPos:clientX-this.startPos;const newFirstSize=this.startFirstSize+delta;const containerSize=this._getContainerSize();const min=this.minPanelSize;const max=containerSize-this.panelGap-min;if(newFirstSize>=min&&newFirstSize<=max){this._setPanelSizes(newFirstSize,containerSize);}}
_setPanelSizes(newFirstSize,containerSize){const newSecondSize=containerSize-newFirstSize-this.panelGap;this.firstPanel.style.height=this.horizontal?`${newFirstSize}px`:"100%";this.secondPanel.style.height=this.horizontal?`${newSecondSize}px`:"100%";this.firstPanel.style.width=this.horizontal?"100%":`${newFirstSize}px`;this.secondPanel.style.width=this.horizontal?"100%":`${newSecondSize}px`;this.divider.ariaValueNow=(newFirstSize/containerSize*100).toFixed(2).toString();}
_onMouseUp(){if(!this.isDragging)return;this.isDragging=false;document.body.style.userSelect="";document.body.style.cursor="";document.removeEventListener("mousemove",this._onMouseMove);document.removeEventListener("touchmove",this._onMouseMove);document.removeEventListener("mouseup",this._onMouseUp);document.removeEventListener("touchend",this._onMouseUp);}
_onDoubleClick(){if(!this.resetOnDoubleClick)return;const containerSize=this.horizontal?this.container.offsetHeight:this.container.offsetWidth;let firstPanelSize=this.firstPanelInitialSize;if(!firstPanelSize){firstPanelSize=containerSize/2-this.panelGap/2;}
this._setPanelSizes(firstPanelSize,containerSize);}
_onTouchEnd(){const now=Date.now();if(now-this.lastTap<300){this._onDoubleClick();}
this.lastTap=now;}
_onKeyDown(e){const key=e.key;let delta=0;const containerSize=this._getContainerSize();if(key.startsWith("Arrow")||key==="Home"||key==="End"){e.preventDefault();}
if(this.horizontal){if(key==="ArrowUp"){delta=-this.keyboardStep;}else if(key==="ArrowDown"){delta=this.keyboardStep;}else if(key==="Home"){this._setPanelSizes(this.minPanelSize,containerSize);}else if(key==="End"){this._setPanelSizes(containerSize-this.panelGap-this.minPanelSize,containerSize);}}else{if(key==="ArrowLeft"){delta=-this.keyboardStep;}else if(key==="ArrowRight"){delta=this.keyboardStep;}else if(key==="Home"){this._setPanelSizes(this.minPanelSize,containerSize);}else if(key==="End"){this._setPanelSizes(containerSize-this.panelGap-this.minPanelSize,containerSize);}}
if(delta!==0){const firstPanelSize=this.horizontal?this.firstPanel.offsetHeight:this.firstPanel.offsetWidth;const newFirstPanelSize=firstPanelSize+delta;const min=this.minPanelSize;const max=containerSize-this.panelGap-min;if(newFirstPanelSize>=min&&newFirstPanelSize<=max){this._setPanelSizes(newFirstPanelSize,containerSize);}}}}
if(!window.mudSplitPanel){window.mudSplitPanel=MudSplitPanel;window.splitPanels={};}
window.mudSplitPanel_update=function(id,horizontal,resetOnDoubleClick,minPanelSize,panelGap){window.splitPanels[id].update(horizontal,resetOnDoubleClick,minPanelSize,panelGap);};window.mudSplitPanel_resetDividerPosition=function(id){window.splitPanels[id].resetSizes();};window.mudSplitPanel_getDividerPosition=function(id){return window.splitPanels[id].getDividerPosition();};window.mudSplitPanel_setDividerPosition=function(id,offset){window.splitPanels[id].setDividerPosition(offset);};window.mudSplitPanel_destroy=function(id){window.splitPanels[id].destroy();};class MudResizeObserverFactory{constructor(){this._maps={};}
...
window.mudInput=new MudInput();"use strict";class MudHotkeyListener{constructor(){this._EVENT_TYPE="keydown";this._hotkeys=new Map();this._handleKeyEventBound=this._handleKeyEvent.bind(this);document.addEventListener(this._EVENT_TYPE,this._handleKeyEventBound);}
dispose(){document.removeEventListener(this._EVENT_TYPE,this._handleKeyEventBound);}
registerOrUpdateHotkey(dotnetRef,dotnetMethodId,hotkeyId,key,modifiers,preventDefault){modifiers=modifiers||[];const newHotkey=this._createHotkey(dotnetRef,dotnetMethodId,hotkeyId,key,modifiers,preventDefault);this._hotkeys.set(hotkeyId,newHotkey);}
unregisterHotkey(hotkeyId){if(this._hotkeys.has(hotkeyId)){this._hotkeys.delete(hotkeyId);}else{console.warn("[MudBlazor] MudHotkey: No matching hotkey found to unregister");}}
_createHotkey(dotnetRef,dotnetMethodId,hotkeyId,key,modifiers,preventDefault){return{dotnetRef:dotnetRef,dotnetMethodId:dotnetMethodId,hotkeyId:hotkeyId,key:key,modifiers:new Set(modifiers),preventDefault:preventDefault};}
_handleKeyEvent(e){const pressedKey=e.code||e.key;const pressedModifiers=this._getPressedModifiers(e);for(const hotkey of this._hotkeys.values()){if(pressedKey!==hotkey.key)continue;const allModifiersPressed=[...hotkey.modifiers].every(m=>pressedModifiers.has(m));const noExtraModifiersPressed=[...pressedModifiers].every(m=>hotkey.modifiers.has(m));if(allModifiersPressed&&noExtraModifiersPressed){if(hotkey.preventDefault){e.preventDefault();}
try{hotkey.dotnetRef.invokeMethodAsync(hotkey.dotnetMethodId);}catch(err){console.error("[MudBlazor] MudHotkey: DotNet invocation failed",{key:hotkey.key,modifiers:[...hotkey.modifiers],err:err});}
break;}}}
_getPressedModifiers(e){const pressedModifiers=new Set();if(e.ctrlKey){if(e.code==="ControlRight"||e.location===KeyboardEvent.DOM_KEY_LOCATION_RIGHT){pressedModifiers.add("ControlRight");}else{pressedModifiers.add("ControlLeft");}}
if(e.shiftKey){if(e.code==="ShiftRight"||e.location===KeyboardEvent.DOM_KEY_LOCATION_RIGHT){pressedModifiers.add("ShiftRight");}else{pressedModifiers.add("ShiftLeft");}}
if(e.altKey){if(e.code==="AltRight"||e.location===KeyboardEvent.DOM_KEY_LOCATION_RIGHT){pressedModifiers.add("AltRight");}else{pressedModifiers.add("AltLeft");}}
if(e.metaKey){if(e.code==="MetaRight"||e.location===KeyboardEvent.DOM_KEY_LOCATION_RIGHT){pressedModifiers.add("MetaRight");}else{pressedModifiers.add("MetaLeft");}}
return pressedModifiers;}}
if(!window.mudHotkeyListener){window.mudHotkeyListener=new MudHotkeyListener();}const darkThemeMediaQuery=window.matchMedia("(prefers-color-scheme: dark)");window.darkModeChange=()=>{return darkThemeMediaQuery.matches;};function darkModeChangeListener(e){dotNetHelperTheme.invokeMethodAsync('SystemDarkModeChangedAsync',e.matches);}
...
(function(){const MIN_RIPPLE_DURATION=500;const FADE_OUT_DURATION=400;function getRippleTarget(event){if(!event.target||!event.target.closest){return null;}
return event.target.closest('.mud-ripple');}
function createRipple(event,target){const rect=target.getBoundingClientRect();const ripple=document.createElement('span');const size=Math.max(rect.width,rect.height)*2;const x=event.clientX-rect.left;const y=event.clientY-rect.top;ripple.className='mud-ripple-effect';ripple.style.width=ripple.style.height=`${size}px`;ripple.style.left=`${x-size/2}px`;ripple.style.top=`${y-size/2}px`;target.appendChild(ripple);ripple.getBoundingClientRect();ripple.classList.add('mud-ripple-effect-expanding');return ripple;}
function removeRipple(ripple,startTime){if(!ripple||!ripple.parentNode){return;}
const elapsed=Date.now()-startTime;const remaining=Math.max(0,MIN_RIPPLE_DURATION-elapsed);setTimeout(function(){ripple.classList.add('mud-ripple-effect-fading');setTimeout(function(){ripple.remove();},FADE_OUT_DURATION);},remaining);}
function handlePointerDown(event){if(event.button!==0){return;}
const target=getRippleTarget(event);if(!target){return;}
const ripple=createRipple(event,target);const startTime=Date.now();if(!target._mudRipples){target._mudRipples=new Map();}
target._mudRipples.set(event.pointerId,{ripple:ripple,startTime:startTime});}
function handlePointerUp(event){const target=getRippleTarget(event);if(!target||!target._mudRipples){return;}
const rippleInfo=target._mudRipples.get(event.pointerId);if(rippleInfo){removeRipple(rippleInfo.ripple,rippleInfo.startTime);target._mudRipples.delete(event.pointerId);}}
function handlePointerLeave(event){const target=getRippleTarget(event);if(!target||!target._mudRipples){return;}
target._mudRipples.forEach(function(rippleInfo){removeRipple(rippleInfo.ripple,rippleInfo.startTime);});target._mudRipples.clear();}
document.addEventListener('pointerdown',handlePointerDown,{passive:true});document.addEventListener('pointerup',handlePointerUp,{passive:true});document.addEventListener('pointercancel',handlePointerUp,{passive:true});document.addEventListener('pointerleave',handlePointerLeave,{passive:true,capture:true});})();class MudScrollManager{constructor(){this._lockCount=0;}
...
window.mudWindow=new MudWindow();window.mudInputSizing={init:(elem,maxLines)=>{const compStyle=getComputedStyle(elem);const lineHeight=parseFloat(compStyle.getPropertyValue('line-height'));const paddingTop=parseFloat(compStyle.getPropertyValue('padding-top'));let maxHeight=0;elem.updateParameters=function(newMaxLines){if(newMaxLines>0){maxHeight=lineHeight*newMaxLines+paddingTop;}else{maxHeight=0;}}
elem.adjustSizingHeight=function(didReflow=false){const scrollTops=[];let curElem=elem;while(curElem&&curElem.parentNode&&curElem.parentNode instanceof Element){if(curElem.parentNode.scrollTop){scrollTops.push([curElem.parentNode,curElem.parentNode.scrollTop]);}
...
elem.style.height=newHeight+"px";scrollTops.forEach(([node,scrollTop])=>{node.style.scrollBehavior='auto';node.scrollTop=scrollTop;node.style.scrollBehavior=null;});if(!didReflow&&initialOverflowY!==elem.style.overflowY&&elem.style.overflowY==='hidden'){elem.style.textAlign='end';elem.adjustSizingHeight(true);}}
elem.restoreToInitialState=function(){elem.removeEventListener('input',elem.adjustSizingHeight);elem.style.overflowY=null;elem.style.height=null;}
elem.addEventListener('input',elem.adjustSizingHeight);window.addEventListener('resize',elem.adjustSizingHeight);elem.updateParameters(maxLines);elem.adjustSizingHeight();},adjustHeight:(elem)=>{if(typeof elem.adjustSizingHeight==='function'){elem.adjustSizingHeight();}},updateParams:(elem,maxLines)=>{if(typeof elem.updateParameters==='function'){elem.updateParameters(maxLines);}
if(typeof elem.adjustSizingHeight==='function'){elem.adjustSizingHeight();}},destroy:(elem)=>{if(elem==null){return;}
window.removeEventListener('resize',elem.adjustSizingHeight);if(typeof elem.restoreToInitialState==='function'){elem.restoreToInitialState();}}};class MudJsEventFactory{connect(dotNetRef,elementId,options){if(!elementId)
...
throw"_options.keys: array of KeyOptions expected";if(this._isConnected){return;}
this._isConnected=true;this._element=element;var targetClass=this._options.targetClass;if(targetClass){this.logger('[MudBlazor | KeyInterceptor] Start observing DOM of element for changes to child with class ',{element,targetClass});this._observer=new MutationObserver(this.onDomChanged);this._observer.mudKeyInterceptor=this;this._observer.observe(this._element,{attributes:false,childList:true,subtree:true});}
this._observedChildren=[];this._keyOptions={};this._regexOptions=[];for(const keyOption of this._options.keys){if(!keyOption||!keyOption.key){this.logger('[MudBlazor | KeyInterceptor] got invalid key options: ',keyOption);continue;}
this.setKeyOption(keyOption)}
this.logger('[MudBlazor | KeyInterceptor] key options: ',this._keyOptions);if(this._regexOptions.size>0)
this.logger('[MudBlazor | KeyInterceptor] regex options: ',this._regexOptions);if(targetClass){for(const child of this._element.getElementsByClassName(targetClass)){this.attachHandlers(child);}}else{this.attachHandlers(this._element);}}
...
disconnect(){if(!this._isConnected)
return;if(this._observer){this.logger('[MudBlazor | KeyInterceptor] disconnect mutation observer and event handlers');this._observer.disconnect();this._observer=null;}
for(const child of this._observedChildren)
this.detachHandlers(child);this._isConnected=false;}
...
var key=args.key.toLowerCase();self.logger('[MudBlazor | KeyInterceptor] down "'+key+'"',args);var invoke=false;if(self._keyOptions.hasOwnProperty(key)){var keyOptions=self._keyOptions[key];self.logger('[MudBlazor | KeyInterceptor] options for "'+key+'"',keyOptions);self.processKeyDown(args,keyOptions);if(self.shouldInvokeKeyDown(args,keyOptions))
invoke=true;}
for(const keyOptions of self._regexOptions){if(keyOptions.regex.test(key)){self.logger('[MudBlazor | KeyInterceptor] regex options for "'+key+'"',keyOptions);self.processKeyDown(args,keyOptions);if(self.shouldInvokeKeyDown(args,keyOptions))
invoke=true;}}
...
shouldInvokeKeyDown(args,keyOptions){return keyOptions.subscribeDown&&(!keyOptions.ignoreDownRepeats||!args.repeat);}
